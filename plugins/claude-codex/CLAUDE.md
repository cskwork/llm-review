...
> **IMPORTANT**: This project uses a skill-based sequential review workflow. The main Claude Code thread handles planning and implementation. Reviews use forked skills (sonnet → codex) that run in isolated contexts.
...
- Sequential reviews: /review-sonnet → /review-codex
...
```
sonnet → fix (if needed) → codex → fix (restart from sonnet if needed)
```
...
- Reviews loop until both reviewers approve
